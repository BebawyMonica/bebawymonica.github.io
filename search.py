...
def breadthFirstSearch(problem):
    ...
    while True:
        if fringe.isEmpty():
            return []
        curState, curPathToLoc, curPathCost = fringe.pop()
        if problem.isGoalState(curState):
            return curPathToLoc
        if curState not in visited:
            visited.add(curState)
            for curSuccess in problem.getSuccessors(curState):
                successState = curSuccess[0]
                successAction = curSuccess[1]
                successActionCost = curSuccess[2]
                if successState not in visited:
                     #if curSuccess not in fringe:
                    succPath = copy.copy(curPathToLoc)
                    succPath.append(successAction)
                    succPathCost = curPathCost + successActionCost
                    succSearchNode = (successState,succPath, succPathCost)
                    fringe.push(succSearchNode)
                #print "Adding to fringe: ", succSearchNode
    ...
    while True:
        ...
        if curState not in visited:
            ...
            for curSuccess in problem.getSuccessors(curState):
                successState = curSuccess[0]
                successPath = curSuccess[1]
                successActionCost = curSuccess[2] + nullHeuristic(curSuccess, problem)
                if successState not in visited:
                    #if curSuccess not in fringe:
                    succPath = copy.copy(curPathToLoc)
                    succPath.append(successPath)
                    hn = heuristic(successState, problem)
                    backCost = curPathCost + successActionCost
                    succPathCost = curPathCost + successActionCost + hn
                    succSearchNode = (successState,succPath, backCost)
                    fringe.push(succSearchNode, succPathCost)
    #util.raiseNotDefined()
